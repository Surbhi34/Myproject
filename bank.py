...
classifier=st.selectbox("Select ML Algorithm or classifier ",("Decision Tree","Logistic Regression",'KNN',"Random Forest","Navye Bayes","Support Vector Machine"))
...
def pre(a,b,c,d,e,f,g,h,i,j,k):
    ...
    if classs =='Decision Tree':
        ...
        if classs=='Logistic Regression':
            logisticRegr = LogisticRegression(solver = 'lbfgs')
            logisticRegr.fit(X_train, y_train)
            result=logisticRegr.predict([[a,b,c,d,e,f,g,h,i,j]])
            a=int(result)
            if a==0:
                st.write(" After anlysing entered information our model predict that  there are maximum chances of the person to stay in the bank")
            else:
            ...
            if classs=='KNN':
                ...
                if classs=='Random Forest':
                    ...
                    if classs=='Navye Bayes':
                        nb = GaussianNB()
                        nb.fit(X_train, y_train)
                        result=nb.predict([[a,b,c,d,e,f,g,h,i,j]])
                        a=int(result)
                        if a==0:
                           st.write(" After anlysing entered information our model predict that  there are maximum chances of the person to stay in the bank")
                        else:
